...
public class HttpClientHelper {
    ...
    public void httpPOST(Context context, String uri, Map params, String action, final AsyncHttpResponseHandler callback) throws UnsupportedEncodingException {
        httpRequest(context, uri, params, action, callback, GET);
    }
    ...
    public void httpRequest(Context context, String uri, Map params, String action, final AsyncHttpResponseHandler callback, int type) {
        //判断网络状态
        ...
        if (!action.equals(NO_COOKIE)) {
            ...
            if (info != null && info.all_header_fields_cookie != null && info.all_header_fields_cookie.length() > 0) {
                BasicClientCookie newCookie = new BasicClientCookie("Cookie", info.all_header_fields_cookie);
