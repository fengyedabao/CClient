				...
				if(mVideoListHead != NULL) {
					...
					if(curTime >= stp) {
						...
						if(mMediaInfo.frame_len > 0) {
							//call back
							if(mCallBack) {
								mCallBack(mMediaInfo.memory_buf, mMediaInfo.frame_len, 0, stp, mUUID, UUID_LEN*2);
							}
				...
				if(mAudioListHead != NULL) {
					...
					if(curTime >= stp) {
						...
						if(mMediaInfo.frame_len > 0) {
							//call back
							if(mCallBack) {
								mCallBack(mMediaInfo.memory_buf, mMediaInfo.frame_len, 1, stp, mUUID, UUID_LEN*2);
							}
						}
					}
				}
...
void AVSyncer::setUUID(unsigned char * uuid, int len)
{
	int uuid_len = len;
	if(uuid_len > UUID_LEN * 2) {
		uuid_len = UUID_LEN * 2;
	}
	memcpy(mUUID, uuid, len);
	mUUID[uuid_len] = '\0';
}
