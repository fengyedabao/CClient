...
static void HandleIPC(unsigned char * pBuf, int size, int type, int timestamp, unsigned char * uuid, int uuidLen)
{
    ...
    if((pBuf[4] & 0x1f) == 0x7) { // sps
        ...
        if(0 == ret) {
            ...
            //__android_log_print(ANDROID_LOG_WARN, "p2p client", "width=%d, height=%d\n", width, height);
            if (1 == get_sps_length(pBuf, size, &spsLen)){
                __android_log_print(ANDROID_LOG_WARN, "p2p client", "sps len=%d\n", spsLen);
                csdInfo = jEnv->NewByteArray(spsLen);
                jEnv->SetByteArrayRegion(csdInfo, 0, (jint)spsLen, (const jbyte*)pBuf);
                jEnv->CallStaticVoidMethod(g_JavaNativeClass, cbUpdateStreamInfo, csdInfo, spsLen, width, height);
                jEnv->DeleteLocalRef(csdInfo);
    ...
    switch(msgID)
    {
        ...
        case 1:
        	...
        	if(g_iIpcCnt%10  == 0){
        		...
        		P2P_sendto((int8*)"keepalive", 9, sAddr, 0, 1);
        ...
        case 2:
        	...
        	__android_log_print(ANDROID_LOG_WARN, "p2p client", "up2p type! type=%d, uuid=%s\n", msgID, uuid_str);
            if(NULL != g_CallRtpSync) {
                g_CallRtpSync->putVideoData(msg, wParam, uuid_str, POS_UUID_LEN * 2);
                if(g_bGetUUID == 0) {
                	g_bGetUUID++;
                	g_CallRtpSync->setUUID(uuid_str, POS_UUID_LEN * 2);
                }
            }
        	break;
        case 3:
 ...
 */
JNIEXPORT jstring JNICALL Java_com_honeywell_cube_ipc_nativeapi_P2PConn_initConn
  (JNIEnv * pEnv, jclass jClass, jstring strPath, jobject pBuf)
{
    ...
    jclass P2PConnClass = pEnv->FindClass("com/honeywell/cube/ipc/nativeapi/P2PConn");
    ...
    ret = P2P_init(g_NCallBack, 0, (int8 *)filePath, 0);
 ...
 * Signature: ()I
 */
JNIEXPORT jint JNICALL Java_com_honeywell_cube_ipc_nativeapi_P2PConn_unInitConn
 ...
 * Signature: (I)I
 */
JNIEXPORT jint JNICALL Java_com_honeywell_cube_ipc_nativeapi_P2PConn_setupConnById
...
JNIEXPORT jint JNICALL Java_com_honeywell_cube_ipc_nativeapi_P2PConn_setupConnByIp
 ...
 * Signature: (I)I
 */
JNIEXPORT jint JNICALL Java_com_honeywell_cube_ipc_nativeapi_P2PConn_disConnById
 ...
 * Signature: ([BII)I
 */
JNIEXPORT jint JNICALL Java_com_honeywell_cube_ipc_nativeapi_P2PConn_sendDataById
  (JNIEnv * pEnv, jclass jClass, jbyteArray sndData, jint dataLen, jstring ssId)
{
	...
	//dstAddr.uuid = ssId;
	memcpy(dstAddr.uuid, uuid_data, POS_UUID_LEN);
	ret = P2P_sendto((int8*)dataArray, dataLen, &dstAddr, 0, 1);
...
JNIEXPORT jint JNICALL Java_com_honeywell_cube_ipc_nativeapi_P2PConn_sendDataByIp
...
// 通话数据，视频
JNIEXPORT jint JNICALL Java_com_honeywell_cube_ipc_nativeapi_P2PConn_sendCallDataVById
  (JNIEnv * pEnv, jclass jClass, jbyteArray sndData, jint dataLen, jstring ssId)
{
	jint ret = -1;
	unsigned char * dataArray;
	...
	//dstAddr.uuid = ssId;
	memcpy(dstAddr.uuid, uuid_data, POS_UUID_LEN);
	ret = P2P_sendto((int8*)dataArray, dataLen, &dstAddr, 0, 2);
    // TODO: 记录uuid，可以在p2p回调时过滤。
...
//通话数据，音频
JNIEXPORT jint JNICALL Java_com_honeywell_cube_ipc_nativeapi_P2PConn_sendCallDataAById
  (JNIEnv * pEnv, jclass jClass, jbyteArray sndData, jint dataLen, jstring ssId)
{
	jint ret = -1;
	unsigned char * dataArray;
	...
	//dstAddr.uuid = ssId;
	memcpy(dstAddr.uuid, uuid_data, POS_UUID_LEN);
	ret = P2P_sendto((int8*)dataArray, dataLen, &dstAddr, 0, 3);
